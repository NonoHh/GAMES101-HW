...
public:
    ...
    bool operator == (const Vector3f& v) { return x - v.x < std::numeric_limits<float>::epsilon() && y - v.y < std::numeric_limits<float>::epsilon() && z - v.z < std::numeric_limits<float>::epsilon(); }
